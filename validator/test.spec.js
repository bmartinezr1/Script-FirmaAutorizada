...
if (rutInput) {
    ...
    try {
        // Intenta primero desde la carpeta actual (validator/)
        let rutFile;
        if (fs.existsSync('ruts_masivos.json')) {
            rutFile = await fs.promises.readFile('ruts_masivos.json', 'utf-8');
        } else if (fs.existsSync('validator/ruts_masivos.json')) {
            rutFile = await fs.promises.readFile('validator/ruts_masivos.json', 'utf-8');
        } else {
            throw new Error('No se encontró ruts_masivos.json');
        }
